...
- [Q 版故事场景 - 黛玉葬花](https://weibo.com/1727858283/PnaUDcKhK)
...
- 主题：图像中有什么或是什么？（例如：一只毛茸茸的三花猫）
- 构图：画面是如何取景的？（例如：特写）
- 动作：正在发生什么？（例如：煮一杯咖啡）
- 地点：场景发生在哪里？（例如：阳光明媚的草地）
- 风格：整体美学风格是什么？（例如：水彩画）
- 编辑指令：如果要修改现有图像，请直接且具体。（例如：移除背景中的汽车）

进阶版应增加：

- 构图与长宽比：定义画布。（例如：9:16 竖版海报）
- 相机与灯光细节：像电影摄影师一样指导拍摄。（例如：黄金时段逆光，营造出长长的阴影）
- 特定文本整合：明确说明应出现哪些文本以及其外观。（例如：标题“URBAN EXPLORER”以粗体、白色、无衬线字体显示在顶部。）
- 事实约束（适用于图表）：明确对准确性的要求，并确保您的输入本身是事实性的。（例如：“确保维多利亚时代的史实准确性。”）
- 参考输入：当使用上传图片时，请明确定义每张图片的作用。（例如：“图片 A 用于角色的姿势，图片 B 用于艺术风格，图片 C 用于背景环境。”）
...
要生成逼真的图像，请像摄影师一样思考。提及相机角度、镜头类型、光照和精细细节将引导模型生成照片级的写实效果。

模板： 一张照片级写实的[拍摄类型]，主体是[主体]，正在[动作或表情]，场景设置在[环境]。场景由[光照描述]照亮，营造出一种[氛围]的氛围。使用[相机/镜头细节]拍摄，强调[关键纹理和细节]。图像应为[宽高比]格式。
示例提示词： 一张照片级写实的特写肖像，主角是一位年迈的日本陶艺家，脸上布满深刻、日晒而成的皱纹，带着温暖而会意的微笑。他正在仔细检查一个刚上釉的茶碗。场景是他那间质朴、阳光普照的工作室。柔和的金色时段光线从窗户射入，照亮了场景，凸显了粘土的细腻纹理。使用85mm人像镜头拍摄，背景柔和模糊（焦外虚化）。整体氛围宁静而富有大师风范。竖向肖像构图。

2. 风格化插画与贴纸

要为您的项目创建贴纸、图标或素材，请明确说明风格，如果需要，请记得要求使用白色背景。

模板： 一张[风格]风格的[主体]贴纸，具有[关键特征]和[色调]。设计应采用[线条风格]和[阴影风格]。背景必须为白色。
示例提示词： 一张卡哇伊风格的贴纸，画的是一只快乐的小熊猫，戴着一顶小小的竹帽。它正在津津有味地吃着一片绿色的竹叶。设计采用粗大、清晰的轮廓，简单的卡通渲染和充满活力的色调。背景必须为白色。

3. 图像中的精准文本

Gemini 擅长渲染文本。请清楚地说明文本内容、字体风格（描述性地）以及整体设计。

模板： 为[品牌/概念]创建一个[图像类型]，其中包含文本“[要渲染的文本]”，字体为[字体风格]。设计应为[风格描述]，配色方案为[配色方案]。
提示词： 为一家名为“The Daily Grind”的咖啡店创建一个现代、简约的徽标。文本应采用干净、粗大的无衬线字体。设计应包含一个与文本无缝集成的简化、风格化的咖啡豆图标。配色方案为黑白。

4. 产品模型图与商业摄影

为电子商务、广告或品牌创建干净、专业的产品照片。

模板： 一张高分辨率、影棚布光的[产品描述]产品照片，放置在[背景表面/描述]上。光照采用[布光设置，例如三点柔光箱设置]以达到[布光目的]。相机角度为[角度类型]以展示[特定特征]。超写实，焦点清晰地对准[关键细节]。[宽高比]。
示例提示词： 一张高分辨率、影棚布光的产品照片，主角是一个哑光黑色的简约陶瓷咖啡杯，放置在抛光的混凝土地面上。光照采用三点柔光箱设置，旨在创造柔和、漫射的高光并消除刺眼的阴影。相机角度为略微抬高的45度角，以展示其简洁的线条。超写实，焦点清晰地对准从咖啡中升起的热气。方形图像。

5. 极简与留白设计

非常适合为网站、演示文稿或需要叠加文本的营销材料创建背景。

模板： 一个极简构图，其中一个[主体]位于画面的[右下角/左上角/等]位置。背景是广阔、空白的[颜色]画布，创造出大量的留白。光线柔和、含蓄。[宽高比]。
示例提示词： 一个极简构图，其中一片精致的红色枫叶位于画面的右下角。背景是广阔、空白的灰白色画布，为文本创造了大量的留白空间。光线从左上角柔和地漫射下来。方形图像。

6. 连续艺术（漫画分镜/故事板）

通过专注于清晰的场景描述，逐帧创建引人入胜的视觉叙事，非常适合开发故事板、连环画或任何形式的连续艺术。

模板： 一个[艺术风格]风格的单幅漫画分镜。前景是[角色描述和动作]。背景是[场景细节]。分镜中有一个带有文字“[文本]”的[对话框/说明框]。光照营造出一种[氛围]的氛围。[宽高比]。
示例提示词： 一个单幅漫画分镜，采用坚毅的黑色电影艺术风格，配以高对比度的黑白墨水。前景中，一位身穿风衣的侦探站在一盏闪烁的路灯下，雨水浸湿了他的肩膀。背景中，一家荒凉酒吧的霓虹灯招牌倒映在水坑里。顶部的说明框写着“这座城市，是藏不住秘密的。” 光线刺眼，营造出一种戏剧化、阴沉的氛围。横向构图。
```

### 证件照

``` txt
截取照片中的人脸，生成新加坡签证的照片，正装，微笑。

截取照片中的人脸，生成1寸照片，蓝底，正装，微笑。

截取图1照片中的人脸，生成1寸照片，正装，微笑。
要求如下：穿校服（校服如图2），不带眼镜。电子照片规格：480像素（宽）*640像素（高）；宽高比为3：4. 蓝色背景。
```
...
> [宝玉xp](https://weibo.com/1727858283/QzXiDrbww)
